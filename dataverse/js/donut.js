function donut(data, div, subset) {

      var donutData = genData(div,data, subset);
    };
    ...
    function DonutCharts(div) {
        ...
        this.create = function(div, dataset) {
            var $charts = $(div);
     ...
     */
    function genData(div, data, subset) {
    	...
    	$.getJSON( data, function( data ) {
	        ...
	        for (var i = 0; i < type.length; i++) {
                var cat = data[type[i]].map(function(d) { return d['name']; });

	            var arr = new Array();
	            var total = 0;

	            for (var j = 0; j < cat.length; j++) {
	            	var result  = data[type[i]].filter(function(o){return o.name == cat[j];} );
	            ...
	            var donuts = new DonutCharts(div);
      			donuts.create(div, dataset.slice(subset[0], subset[1]));	
